...
BACKUP_FILE="${BACKUP_DIR}/sonaro_${TIMESTAMP}.sql.gz"
...
find "${BACKUP_DIR}" -name "sonaro_*.sql.gz" -type f -mtime +${RETENTION_DAYS} -delete

# List remaining backups
TOTAL=$(ls -1 ${BACKUP_DIR}/sonaro_*.sql.gz 2>/dev/null | wc -l)
