#!/bin/bash
# ============================================================
# Sonaro Gate Agent - Installer for Ubuntu 24.04 LTS
...
echo -e "${CYAN}"
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║     Sonaro Gate Agent Installer v3.0              ║"
...
detect_nics() {
  ...
  if [[ $nic_count -eq 0 ]]; then
    ...
    echo -e "  ${YELLOW}  Sonaro Gate requires at least 2 NICs for proper operation:${NC}"
...
mkdir -p "$SONARO_DIR"/{rules,backups}

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cp "$SCRIPT_DIR/sonaro-agent.sh" "$SONARO_DIR/sonaro-agent.sh"
chmod +x "$SONARO_DIR/sonaro-agent.sh"
...
cat > "$SONARO_DIR/.env" <<EOF
# ============================================================
# Sonaro Gate Agent Configuration v3.0
...
echo -e "  ${GREEN}✓${NC} Config saved to ${CYAN}$SONARO_DIR/.env${NC}"

echo -e "${GREEN}[7/8]${NC} Installing systemd service & log rotation..."
cp "$SCRIPT_DIR/sonaro-agent.service" /etc/systemd/system/sonaro-agent.service
systemctl daemon-reload

cat > /etc/logrotate.d/sonaro-agent <<'LOGROTATE'
/opt/sonaro/agent.log {
    daily
    missingok
    rotate 14
    compress
    ...
    postrotate
        systemctl reload sonaro-agent 2>/dev/null || true
    endscript
}
LOGROTATE
...
if command -v docker &>/dev/null; then
  docker exec -i aegis-db psql -U "${POSTGRES_USER:-aegis}" -d "${POSTGRES_DB:-sonaro_gate}" <<'GRANTS' 2>/dev/null || true
...
echo -e "\n  ${CYAN}Starting Sonaro Gate Agent service...${NC}"
systemctl enable sonaro-agent 2>/dev/null || true
systemctl restart sonaro-agent 2>/dev/null || true
if systemctl is-active --quiet sonaro-agent; then
  echo -e "  ${GREEN}✓${NC} Agent service started and enabled"
else
  echo -e "  ${YELLOW}○${NC} Agent service may need manual start: systemctl start sonaro-agent"
fi
...
echo ""
echo -e "${GREEN}════════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}  Sonaro Gate Agent v3.0 — Installation Complete${NC}"
echo -e "${GREEN}════════════════════════════════════════════════════════${NC}"
echo ""
...
echo -e "    1. Review config:    ${CYAN}nano $SONARO_DIR/.env${NC}"
echo -e "    2. Test connection:  ${CYAN}$SONARO_DIR/sonaro-agent.sh status${NC}"
echo -e "    3. Dry-run test:     ${CYAN}$SONARO_DIR/sonaro-agent.sh test${NC}"
echo -e "    4. Start service:    ${CYAN}systemctl enable --now sonaro-agent${NC}"
echo -e "    5. View logs:        ${CYAN}tail -f $SONARO_DIR/agent.log${NC}"
