#!/bin/bash
# ============================================================
# Sonaro Gate Agent - Ubuntu System Monitor & Firewall Sync
...
#
# Usage:
#   ./sonaro-agent.sh daemon         # Run as background service
#   ./sonaro-agent.sh sync           # One-time full config sync
#   ./sonaro-agent.sh metrics        # Collect & push metrics once
#   ./sonaro-agent.sh status         # Show agent status
#   ./sonaro-agent.sh fetch          # Fetch rules and display
#   ./sonaro-agent.sh backup         # Backup current rules
#   ./sonaro-agent.sh apply-fw       # Apply firewall rules only
#   ./sonaro-agent.sh apply-nat      # Apply NAT rules only
#   ./sonaro-agent.sh apply-routes   # Apply static + policy routes
#   ./sonaro-agent.sh apply-tc       # Apply traffic shaping only
#   ./sonaro-agent.sh apply-iface    # Apply interface config only
#   ./sonaro-agent.sh apply-dhcp     # Apply DHCP config only
#   ./sonaro-agent.sh apply-dns      # Apply DNS config only
#   ./sonaro-agent.sh apply-ids      # Apply IDS/IPS (Suricata) rules
#   ./sonaro-agent.sh apply-av       # Apply AntiVirus (ClamAV) config
#   ./sonaro-agent.sh apply-webfilter    # Apply Web Filter (Squid)
#   ./sonaro-agent.sh apply-dnsfilter    # Apply DNS Filter blocklist
#   ./sonaro-agent.sh apply-appcontrol   # Apply Application Control
#   ./sonaro-agent.sh capture-start ID   # Start packet capture
#   ./sonaro-agent.sh capture-stop ID    # Stop packet capture
#   ./sonaro-agent.sh discover       # Network topology discovery
#   ./sonaro-agent.sh firmware-info  # Collect firmware info
#   ./sonaro-agent.sh config-backup  # Create config backup
#   ./sonaro-agent.sh config-restore # Restore config backup
#   ./sonaro-agent.sh test           # Dry-run (generate but don't apply)
#
...
# ── Configuration ────────────────────────────────────────────
SONARO_DIR="${SONARO_DIR:-/opt/sonaro}"
ENV_FILE="${SONARO_DIR}/.env"
...
LOG_FILE="${SONARO_DIR}/agent.log"
RULES_DIR="${SONARO_DIR}/rules"
PID_FILE="${SONARO_DIR}/agent.pid"
...
# ── Initialization ───────────────────────────────────────────
init() {
  mkdir -p "$RULES_DIR" "${SONARO_DIR}/backups"

  log_info "════════════════════════════════════════════════"
  log_info "  Sonaro Gate Agent v2.0 Starting"
...
collect_traffic_stats() {
  ...
  if [[ "$FIREWALL_BACKEND" == "nft" ]]; then
    blocked=$(nft list chain inet sonaro_filter input 2>/dev/null | grep -c "drop" || echo 0)
  else
...
collect_threats() {
  ...
  local last_check_file="${SONARO_DIR}/.last_threat_check"
...
#!/bin/bash
set -e
# Auto-generated by Sonaro Gate Agent — DO NOT EDIT
iptables -F
iptables -X
iptables -t nat -F
iptables -t nat -X
...
HEADER
  ...
  echo "$rules" | jq -c '.[]' | while read -r rule; do
    ...
    if [[ "$logging" == "true" ]]; then
      echo "${cmd} -j LOG --log-prefix 'SONARO_${order}: ' --log-level 4" >> "$script"
    fi
...
apply_nft_rules() {
  local rules="$1"
  local nft_file="${RULES_DIR}/sonaro.nft"

  cat > "$nft_file" <<HEADER
#!/usr/sbin/nft -f
# Auto-generated by Sonaro Gate Agent v2.0 — DO NOT EDIT
# Generated: $(date -u +%Y-%m-%dT%H:%M:%SZ)
flush ruleset

table inet sonaro_filter {
  chain input {
...
#!/bin/bash
set -e
# NAT rules — auto-generated by Sonaro Gate Agent
...
#!/bin/bash
set -e
# Traffic Shaping — auto-generated by Sonaro Gate Agent
...
# Sonaro Gate DHCP Configuration — auto-generated
# DO NOT EDIT MANUALLY — managed by sonaro-agent
HEADER
...
apply_dhcp_isc() {
  ...
  cat > "$conf_file" <<'HEADER'
# Sonaro Gate DHCP Configuration (ISC DHCP) — auto-generated
...
apply_dns_dnsmasq() {
  ...
  cat > "$conf_file" <<'HEADER'
# Sonaro Gate DNS Configuration — auto-generated
# DO NOT EDIT MANUALLY
HEADER
...
apply_dns_bind9() {
  ...
  cat > "$conf_file" <<'HEADER'
// Sonaro Gate DNS Configuration (BIND9) — auto-generated
// DO NOT EDIT MANUALLY
HEADER
...
; Sonaro Gate Local DNS Zone — auto-generated
\$TTL 300
...
apply_ids_config() {
  ...
  echo "# Sonaro Gate Custom IDS Rules — auto-generated" > "$rules_file"
...
# Sonaro Gate ClamAV Scan Configuration — auto-generated
# DO NOT EDIT MANUALLY — managed by sonaro-agent
HEADER
...
# Auto-generated by Sonaro Gate Agent — $name
...
# Sonaro Gate Web Filter Configuration — auto-generated
# DO NOT EDIT MANUALLY — managed by sonaro-agent
HEADER
...
apply_dnsfilter_config() {
  ...
  cat > "$conf_file" <<'HEADER'
# Sonaro Gate DNS Filter Configuration — auto-generated
# DO NOT EDIT MANUALLY — managed by sonaro-agent
HEADER
  ...
  local blocklist_dir="${SONARO_DIR}/blocklists"
...
apply_appcontrol_config() {
  ...
  echo "# Sonaro Gate Application Control Rules — auto-generated" > "$rules_file"
  echo "# Generated: $(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$rules_file"
  echo "" >> "$rules_file"
...
backup_rules() {
  ...
  local backup_dir="${SONARO_DIR}/backups"
...
start_packet_capture() {
  ...
  local pcap_dir="${SONARO_DIR}/captures"
  mkdir -p "$pcap_dir"
  local ts=$(date +%Y%m%d_%H%M%S)
...
create_config_backup() {
  ...
  local notes="${2:-}"
  local backup_dir="${SONARO_DIR}/backups/configs"
  mkdir -p "$backup_dir"
  ...
  # Agent config
  cp "${SONARO_DIR}/.env" "${temp_dir}/agent.env" 2>/dev/null || true
...
run_daemon() {
  ...
  while true; do
    ...
    if (( now - last_sync >= SYNC_INTERVAL )); then
      ...
      local old_hash=""
      [[ -f "${SONARO_DIR}/.rules_hash" ]] && old_hash=$(cat "${SONARO_DIR}/.rules_hash")

      if [[ "$new_hash" != "$old_hash" ]]; then
        log_info "Rule changes detected, syncing..."
        do_sync || log_warn "Sync failed"
        echo "$new_hash" > "${SONARO_DIR}/.rules_hash"
      fi
      last_sync=$now
    fi
...
show_status() {
  echo -e "${CYAN}═══════════════════════════════════════${NC}"
  echo -e "${CYAN}   Sonaro Gate Agent v2.0 — Status     ${NC}"
  echo -e "${CYAN}═══════════════════════════════════════${NC}"
...
case "${1:-daemon}" in
  ...
  test)
    ...
    ;;
  *)
    echo "Sonaro Gate Agent v3.0 — Self-Hosted Firewall Management"
    echo ""
